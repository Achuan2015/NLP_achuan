...
def run():
    ...
    dfs_valid = pd.read_csv(path_eval, sep='\t')
    ...
    dataset_train = MyDataset(
        inputs=dfs_train.review_clean.values,
        targets=dfs_train.label.values
    )

    dataset_valid = MyDataset(
        inputs=dfs_valid.review_clean.values,
        targets=dfs_valid.label.values
    )
